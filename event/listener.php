...
class listener implements EventSubscriberInterface
{
	...
	*/
	...
	public function kem_vi($event)
	{
		...
		$vi_gd_status = $this->config->offsetGet('vi_gd_status');
		if ($vi_gd_status == 0)
		{
		...
		if (preg_match('/^download\/file\.php\?id=\d/i', $row['session_page']) and $this->auth->acl_get('u_kemrash_vi'))
		{
			$clear_id = preg_replace('/(^.*?id=|\D.*)/i', '', $row['session_page']);
			...
			{
				if ($vi_gd_status == 1 and ($mimetype['mimetype'] == 'image/jpeg' or $mimetype['mimetype'] == 'image/jpg' or $mimetype['mimetype'] == 'image/png') and $mimetype['physical_filename'] != NULL)
				{
					$img_url = 'ext/kemrash/vi/styles/all/theme/images/' . $clear_id;
					$img_url_origen = $this->phpbb_root_path . $this->config->offsetGet('upload_path') . '/' . $mimetype['physical_filename'];
					...
					elseif (is_writable($dir) and !file_exists($img_url . '.jpg') and ($mimetype['mimetype'] == 'image/jpeg' or $mimetype['mimetype'] == 'image/jpg'))
					{
						...
						{
							$location =  $location . ' <img src="download/file.php?id=' . $clear_id . '" style=" max-width: 100px; max-height: 120px; ">';
						}
					}
